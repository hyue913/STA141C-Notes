...
project <- "exemplary-vista-269321"
```
...
result <- bq_project_query(
  project,
  "SELECT * FROM `exemplary-vista-269321.mydataset.mtcars` where `mpg` < 30")
bq_table_download(result)
```
...
*DBI*
```{r, eval = FALSE}
con %>% dbGetQuery("SELECT * FROM `exemplary-vista-269321.mydataset.mtcars` WHERE `mpg` < 30")
```
...
```{sql connection = con, eval = FALSE}
CREATE OR REPLACE TABLE `exemplary-vista-269321.mydataset.mtcars2` AS
  SELECT *,
  RAND() < 0.9 as `train`
  FROM `exemplary-vista-269321.mydataset.mtcars`
```


```{sql connection = con, eval = FALSE}
CREATE OR REPLACE MODEL `exemplary-vista-269321.mydataset.mtcars_model`
OPTIONS
...
FROM
  `exemplary-vista-269321.mydataset.mtcars2`
WHERE
  `train` = true
```
...
```{sql connection = con, eval = FALSE}
DROP MODEL `exemplary-vista-269321.mydataset.mtcars_model`;
```
...
CREATE OR REPLACE VIEW
  `exemplary-vista-269321.spam_view` AS
SELECT
  `all` AS `a`,
  `over` AS `o`,
  `order` AS `ord`,
  * EXCEPT (`all`, `over`, `order`),
  ...
  END AS dataframe
FROM
  `exemplary-vista-269321.mydataset.spam`
```
